...
function FileDiffRow({ file, commitHash, selectedProject }) {
  ...
  return (
    ...
    <div
        ...
        tabIndex={0}
        aria-expanded={open}
        className={`w-full flex items-center gap-3 px-3 py-2.5 hover:bg-(--bg-muted)/40 text-xs cursor-pointer transition-all
          ${open ? 'bg-(--bg-muted)/40' : ''}`}
        onClick={handleExpand}
        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleExpand(); } }}
      >
        <span className={`shrink-0 w-4.5 h-4.5 flex items-center justify-center text-[10px] font-bold rounded shadow-sm ${STATUS_CONFIG[file.status] || ''}`}>
          {file.status}
        </span>
        <span className="flex-1 font-mono text-left truncate min-w-0">
          <span className="text-(--text-secondary) opacity-70 text-[11px]">{dir}</span>
          <span className="text-(--text-primary) font-semibold opacity-85">{fname}</span>
        </span>
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <div className={`ml-1 transition-transform duration-300 ${open ? 'rotate-90 text-(--git-accent)' : 'opacity-40'}`}>
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.div
            ...
            className={`overflow-hidden ${isDarkMode ? 'bg-(--bg-base)/30' : 'bg-slate-50'}`}
          >
            <div className="border-t border-border/40">
              <div className={`flex items-center justify-between px-4 py-2 border-b border-border/20 ${isDarkMode ? 'bg-(--bg-surface)/10' : 'bg-white'}`}>
                <span className="text-[9px] text-(--text-secondary) font-mono truncate opacity-60 tracking-wider uppercase">{file.filePath}</span>
              </div>
              <div className="overflow-hidden shadow-inner">
                {loadingDiff ? (
                  <div className="p-4 text-center text-xs text-(--text-secondary) animate-pulse">Loading diff...</div>
                ) : diffError ? (
                  ...
                  <div className="p-4 text-center text-xs text-(--text-secondary)">No diff available</div>
                )}
              </div>
            </div>
          </motion.div>
...
export default function CommitDetail() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-(--bg-base)">
      {/* Header / Metadata */}
      {commitMeta && (
        <div className="p-4 shrink-0 space-y-4 bg-(--bg-surface)/20 border-b border-border">
          <div className="space-y-2">
            ...
            >
              <h1 className="text-[15.5px] font-bold text-(--text-primary) opacity-90 leading-snug whitespace-pre-wrap selection:bg-(--git-accent)/30">
                {commitMeta.message || ''}
              </h1>
              {!msgExpanded && (commitMeta.message?.length ?? 0) > 160 && (
                <div className="absolute bottom-0 left-0 right-0 h-4 bg-linear-to-t from-(--bg-base) to-transparent pointer-events-none" />
              )}
            </motion.div>
            ...
            {(commitMeta.message?.length ?? 0) > 160 && (
              <button
                ...
                className="text-[10px] font-bold text-(--git-accent) hover:underline uppercase tracking-widest"
              >
          ...
          <div className="flex flex-wrap items-center gap-y-3 gap-x-6 pt-2">
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                <p className="text-[13px] font-bold text-(--text-primary) opacity-90 truncate leading-none">{commitMeta.author}</p>
                <div className="flex items-center min-w-0">
                  <Tooltip label={commitMeta.email || ''}>
                    <p className="text-[12px] text-(--text-secondary) leading-normal opacity-60 truncate">
                      {(() => {
                  ...
                  {commitMeta.email && (
                    <Tooltip label="Copy email">
                      <button 
                        ...
                        className="p-1.5 hover:text-(--git-accent) transition-colors opacity-40 hover:opacity-100"
                      >
            ...
            <div className="flex items-center gap-4 ml-auto">
              <div className="text-right">
                {(() => {
                  ...
                  if (!isValid) return <p className="text-[13px] font-mono text-(--text-secondary) leading-none mb-1">—</p>;
                  ...
                  return (
                    <Tooltip label={tooltip} contentClassName="text-[12px] px-3 py-2">
                      <p className="text-[13px] font-mono text-(--text-secondary) leading-none mb-1">
                        {display}
                      </p>
                    </Tooltip>
                  );
                ...
                <div className="flex items-center justify-end gap-1.5 text-[12px] font-mono opacity-80">
                  <Tooltip label={selectedCommit} contentClassName="font-mono text-[11px]">
                    <span className="text-(--git-accent)">{selectedCommit.slice(0, 8)}</span>
                  </Tooltip>
                  <Tooltip label="Copy hash">
                    <button onClick={() => copyToClipboard(selectedCommit)} className="p-1.5 hover:text-(--git-accent) transition-colors">
                      <Copy className="w-3.5 h-3.5" />
                    </button>
                  </Tooltip>
                </div>
          ...
          {files.length > 0 && (
            <div className="flex items-center gap-3 pt-2 text-[10px] font-bold uppercase tracking-widest border-t border-border/20 mt-2 text-(--text-secondary)">
      ...
      <div className="flex-1 overflow-y-auto min-h-0 scrollbar-none">
        {error?.action === 'commit-diff' ? (
          ...
          <div className="h-40 flex items-center justify-center">
            <div className="text-[10px] font-bold tracking-[0.2em] text-(--text-secondary) uppercase animate-pulse opacity-40">Loading changes…</div>
          </div>
        ) : files.length === 0 ? (
          <div className="p-12 flex flex-col items-center justify-center gap-4 text-center opacity-40">
             <div className="w-12 h-12 rounded-full border-2 border-dashed border-(--text-secondary) flex items-center justify-center text-xl font-thin">∅</div>
             <p className="text-[10px] text-(--text-secondary) font-bold uppercase tracking-[0.2em]">No lines were modified</p>
          </div>
        ) : (
          <div className="pb-12">
            {files.map(f => (
