...
export default function BranchBar() {
  ...
  const hoverBgClass = isDarkMode ? 'hover:bg-(--bg-muted)' : 'hover:bg-slate-200/60';
  ...
  return (
    <div className="flex items-center gap-1.5 px-3 py-1.5 h-[3.3rem] bg-(--bg-base) shrink-0 transition-colors border-b border-border">
      ...
      >
        <button
          ...
          className={`flex items-center gap-1 px-2.5 py-1.5 rounded-sm bg-transparent text-[10px] font-bold text-(--git-accent) transition-all active:scale-95 whitespace-nowrap uppercase tracking-tighter
            ${hoverBgClass}`}
        >
      ...
      <div className="relative min-w-0" ref={dropdownRef}>
        <button
          ...
          className={`flex items-center gap-1.5 px-2 py-1 rounded-md text-sm max-w-full
            ...
            ${isDetachedHead ? 'text-orange-400' : 'text-(--text-primary)'}`}
        >
          {isDetachedHead
            ? <AlertTriangle className="w-3.5 h-3.5 shrink-0 text-orange-400" />
            : <GitBranch className="w-5 h-5 shrink-0 text-(--git-accent)" />}
          <TruncatedTooltip 
            label={isDetachedHead ? 'DETACHED HEAD' : currentBranch} 
            className="font-mono text-xs font-semibold max-w-30"
          >
          ...
          <ChevronDown className={`w-3 h-3 shrink-0 text-(--text-secondary) transition-transform ${showDropdown ? 'rotate-180' : ''}`} />
        </button>
        ...
        <AnimatePresence>
          {showDropdown && (
            <motion.div 
              ...
              className="absolute top-full left-0 mt-1 w-64 bg-(--bg-surface) rounded-lg shadow-2xl z-60 overflow-hidden border border-border/50"
            ...
            >
              ...
              <div className="p-2 border-b border-border/40">
                <input
                  ...
                  className="w-full bg-(--bg-base) rounded px-2 py-1.5 text-xs text-(--text-primary) placeholder-(--text-secondary) outline-none focus:ring-1 focus:ring-(--git-accent)/30 transition-all font-medium"
                />
              </div>
              <div className="max-h-48 overflow-y-auto py-1">
                {filteredBranches.map(branch => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 text-xs text-left ${hoverBgClass} transition-colors group disabled:opacity-50
                      ${branch === currentBranch ? 'text-(--accent) bg-(--accent)/5' : 'text-(--text-primary)'}`}
                  >
                ...
                {filteredBranches.length === 0 && (
                  <p className="px-3 py-4 text-xs text-(--text-secondary) text-center italic">No branches found</p>
                )}
              </div>
              <div className="p-1 border-t border-border/40 bg-(--bg-muted)/30">
                {!showNewBranch ? (
                  <button
                    onClick={() => setShowNewBranch(true)}
                    className={`w-full flex items-center gap-2 px-3 py-2 text-xs text-(--text-secondary) hover:text-(--text-primary) ${hoverBgClass} rounded transition-colors group`}
                  >
                    <Plus className="w-3.5 h-3.5 group-hover:text-(--git-accent) transition-colors" />
                  ...
                  <div className="flex items-center gap-1 p-1">
                    <input
                      ...
                      className="flex-1 bg-(--bg-base) border border-border/60 rounded px-2 py-1.5 text-xs font-mono text-(--text-primary) placeholder-(--text-secondary) outline-none focus:border-(--git-accent)/50 transition-all font-semibold"
                    />
                    <button
                      onClick={handleCreateBranch}
                      disabled={creatingBranch || !newBranchName.trim()}
                      className="p-1.5 rounded bg-(--git-accent) text-white hover:opacity-90 disabled:opacity-40 transition-colors shadow-lg shadow-(--git-accent)/20"
                    >
                      <Check className="w-3.5 h-3.5" />
                    </button>
      ...
      <div className="flex items-center gap-0.5 ml-auto shrink-0">
        ...
        <Tooltip label="Sync with Remote & Refresh">
          <button
            ...
            className={`p-1.5 rounded text-(--text-secondary) ${hoverBgClass} disabled:opacity-40 transition-colors pointer-events-auto`}
          >
