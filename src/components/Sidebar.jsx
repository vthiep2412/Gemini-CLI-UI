...
// Global clock hook for components that need to update based on time without causing
// parent renders. Updates every minute.
let globalTimeListeners = new Set();
let globalTimeInterval = null;

const getGlobalTime = () => new Date();
  ...
  useEffect(() => {
    ...
    if (!globalTimeInterval) {
      globalTimeInterval = setInterval(() => {
        ...
        globalTimeListeners.forEach(l => l(now));
      }, 60000); // 1 minute
    }

    return () => {
      globalTimeListeners.delete(listener);
      if (globalTimeListeners.size === 0 && globalTimeInterval) {
