  ...
  try {
    ...
    if (files && files.length > 0) {
      ...
      if (validFiles.length > 0) {
        // Optimize: Batch files into a single git add command to avoid N+1 subprocess bottleneck
        await execFileAsync('git', ['add', '--', ...validFiles], { cwd: projectPath });
      }
    }
...
router.post('/unstage', async (req, res) => {
  ...
  try {
    ...
    if (files === 'all' || !files) {
      ...
      if (validFiles.length > 0) {
        // Optimize: Batch files into a single git restore command to avoid N+1 subprocess bottleneck
        await execFileAsync('git', ['restore', '--staged', '--', ...validFiles], { cwd: projectPath });
      }
    }
    res.json({ success: true });
  } catch (error) {
