...
async function getFileTree(dirPath, maxDepth = 3, currentDepth = 0, showHidden = true) {
  ...
  try {
    ...
    // Skip heavy build directories early
    ...
    // Process entries in chunks to optimize I/O via Promise.all while preventing EMFILE
    const chunkSize = 20;
    for (let i = 0; i < entries.length; i += chunkSize) {
      const chunk = entries.slice(i, i + chunkSize);
      
      // Phase 1: Parallel stat fetches for the chunk
      const chunkItems = await Promise.all(chunk.map(async (entry) => {
        ...
        const item = {
          ...
          entryObj: entry // Keep entry for phase 2
        };
      ...
      // Phase 2: Sequential recursion to avoid exponential concurrency
      for (const item of chunkItems) {
        if (item.entryObj.isDirectory() && currentDepth < maxDepth) {
          try {
        ...
        delete item.entryObj; // Clean up temporary reference
        items.push(item);
      }
    }
  } catch (error) {
